...
LEGACY_RELEASE_CONTRACT = release_contract
...
AUTHOR_PLACEHOLDER = re.compile(
    r"(?<![A-Za-z])(?:anonymous|placeholder|tbd|todo|pending|"
    r"internal[\s_-]*review|author\s*\(s\)|your\s+name|first\s+author)"
    r"(?![A-Za-z])",
    flags=re.IGNORECASE,
)
...
V2_CONTRACT_MARKER = r"\HeadlineHardAThreePrimeMacroFOne"


def _configure_release_contract(contract: Any) -> None:
    """Select the v1 or v2 portable macro/lock contract for one audit."""

    global release_contract
    global RELEASE_LOCK_SCHEMA
    global RELEASE_SENTINEL
    global RELEASE_SENTINEL_VALUE
    global FORMAL_CACHE_DESIGNATION
    global PROVENANCE_MACROS
    global HEADLINE_NUMERIC_MACROS
    global REGIME_TOKENS
    global REGIME_NUMERIC_MACROS
    global ABLATION_MEAN_NUMERIC_MACROS
    global ABLATION_CONTRAST_PREFIXES
    global ABLATION_CONTRAST_NUMERIC_MACROS
    global ABLATION_TABLE_NUMERIC_MACROS
    global MECHANISM_NUMERIC_MACROS
    global VIMD_NUMERIC_MACROS
    global NUMERIC_RESULT_MACROS
    global RESULT_MACROS
    global PLACEHOLDER_TERMS

    release_contract = contract
    RELEASE_LOCK_SCHEMA = contract.RELEASE_LOCK_SCHEMA
    RELEASE_SENTINEL = contract.RELEASE_SENTINEL
    RELEASE_SENTINEL_VALUE = contract.RELEASE_SENTINEL_VALUE
    FORMAL_CACHE_DESIGNATION = contract.FORMAL_DESIGNATION
    PROVENANCE_MACROS = tuple(contract.PROVENANCE_MACROS)
    HEADLINE_NUMERIC_MACROS = tuple(
        contract.HEADLINE_PROVENANCE_MACROS
    )
    REGIME_TOKENS = tuple(contract.REGIME_TOKENS)
    REGIME_NUMERIC_MACROS = tuple(
        contract.REGIME_PROVENANCE_MACROS
    )
    ABLATION_MEAN_NUMERIC_MACROS = tuple(
        contract.ABLATION_MEAN_PROVENANCE_MACROS
    )
    ABLATION_CONTRAST_PREFIXES = tuple(
        contract.ABLATION_CONTRAST_PREFIXES
    )
    ABLATION_CONTRAST_NUMERIC_MACROS = tuple(
        contract.ABLATION_CONTRAST_PROVENANCE_MACROS
    )
    ABLATION_TABLE_NUMERIC_MACROS = (
        *ABLATION_MEAN_NUMERIC_MACROS,
        *ABLATION_CONTRAST_NUMERIC_MACROS,
    ...
    MECHANISM_NUMERIC_MACROS = tuple(
        contract.MECHANISM_PROVENANCE_MACROS
    )
    VIMD_NUMERIC_MACROS = tuple(
        name
        for name in PROVENANCE_MACROS
    ...
    RESULT_MACROS = tuple(contract.NON_SENTINEL_RESULT_MACROS)
    PLACEHOLDER_TERMS = contract.PLACEHOLDER_TERMS


def _select_release_contract(results_text: str) -> None:
    if V2_CONTRACT_MARKER not in results_text:
        _configure_release_contract(LEGACY_RELEASE_CONTRACT)
        return
    try:
        from tvt_submission import validate_v2_paper_release
    except ModuleNotFoundError:
        import validate_v2_paper_release
    _configure_release_contract(validate_v2_paper_release)
...
def _ablation_table_contract_errors(public_text: str) -> list[str]:
    ...
    if not expected_macros or len(set(expected_macros)) != len(
        expected_macros
    ):
        errors.append(
            "canonical release contract must export a non-empty, unique "
            "ablation-table macro inventory"
        )
    ...
    if wrong_counts:
        errors.append(
            ...
            f"{len(expected_macros)} canonical ablation macros exactly "
            "once; observed "
            + ",".join(wrong_counts)
        )
    return errors
...
def _release_result_source_identity_errors(
    lock: dict[str, Any],
    macros: dict[str, str],
) -> list[str]:
    renderer = getattr(release_contract, "result_source_value", None)
    if not callable(renderer):
        return []
    try:
        expected = renderer(lock)
    except (KeyError, TypeError, ValueError) as error:
        return [
            "release lock cannot render the canonical ResultSource: "
            f"{error}"
        ]
    if macros.get("ResultSource") != expected:
        return [
            "ResultSource does not identify the exact release-lock run_id "
            "and cache digest prefix"
        ]
    return []
...
def _braced_command_arguments(
    text: str,
    command: str,
) -> tuple[list[str], list[str]]:
    """Extract balanced braced arguments for one exact TeX command."""

    pattern = re.compile(
        rf"\\{re.escape(command)}(?![A-Za-z@])\s*\{{"
    )
    arguments: list[str] = []
    errors: list[str] = []
    for match in pattern.finditer(text):
        ...
        while cursor < len(text) and depth:
            ...
            cursor += 1
        if depth:
            errors.append(rf"unterminated \{command} argument")
            continue
        arguments.append(text[start : cursor - 1])
    return arguments, errors


def _author_metadata_errors(text: str) -> list[str]:
    """Reject public authorship placeholders and empty author blocks."""

    active = "\n".join(
        _strip_tex_comment(line) for line in text.splitlines()
    )
    errors: list[str] = []
    findings = _placeholder_findings(active, AUTHOR_PLACEHOLDER)
    if findings:
        errors.append(
            "authors_verified.tex contains placeholder authorship text: "
            + "; ".join(
                f"line {item['line_number']}: {item['text']}"
                for item in findings[:5]
            )
        )
    arguments, parse_errors = _braced_command_arguments(active, "author")
    errors.extend(parse_errors)
    if len(arguments) != 1:
        errors.append(
            "authors_verified.tex must contain exactly one balanced "
            r"\author{...} command"
        )
        return errors
    name_region = arguments[0].split(r"\thanks", maxsplit=1)[0]
    visible = re.sub(r"\\[A-Za-z@]+", " ", name_region)
    tokens = re.findall(r"[A-Za-z][A-Za-z'’-]*", visible)
    if len(tokens) < 2 or sum(len(token) for token in tokens) < 4:
        errors.append(
            "authors_verified.tex does not contain a substantive public "
            "author name before any author notes"
        )
    return errors
...
def audit_paper_build(
    *,
    ...
    # Tests and long-lived callers may audit v1 and v2 papers in one process.
    # Start and finish from the historical default so contract selection never
    # leaks across independent audits.
    _configure_release_contract(LEGACY_RELEASE_CONTRACT)
    ...
    expected_main_path = (root / "main.tex").resolve()
    _record_check(
        report,
        "main_path_binding",
        paths["main_tex"] == expected_main_path,
        actual=str(paths["main_tex"]),
        required=str(expected_main_path),
        failure=(
            "the audited main source is not paper_root/main.tex"
        ),
    )
    _record_check(
        report,
    ...
    if normalized_mode == "release":
        expected_lock_path = (root / "release_lock.json").resolve()
        _record_check(
            report,
            "release_lock_path_binding",
            paths["release_lock"] == expected_lock_path,
            actual=str(paths["release_lock"]),
            required=str(expected_lock_path),
            failure=(
                "the audited release lock is not "
    ...
    _select_release_contract(decoded.get("results_auto", ""))
    ...
    if main_text is not None:
        ...
        if normalized_mode == "release":
            authors_path = (root / "authors_verified.tex").resolve()
            authors_declared = _path_key(authors_path) in {
                _path_key(path) for path in declared_dependencies
            }
            author_errors: list[str] = []
            if not authors_declared:
                author_errors.append(
                    "public main.tex does not include the canonical "
                    "paper_root/authors_verified.tex"
                )
            else:
                try:
                    authors_artifact = _stable_read(
                        authors_path,
                        "verified public author metadata",
                    )
                    authors_text = _decode_utf8(
                        authors_artifact,
                        "verified public author metadata",
                    )
                except PaperBuildValidationError as error:
                    author_errors.append(str(error))
                else:
                    author_errors.extend(
                        _author_metadata_errors(authors_text)
                    )
            report["author_metadata_errors"] = author_errors
            _record_check(
                report,
                "release_author_metadata",
                not author_errors,
                actual=author_errors,
                required=(
                    "canonical authors_verified.tex with one substantive, "
                    "non-placeholder public author block"
                ),
                failure="; ".join(author_errors),
            )
    ...
    if normalized_mode == "release" and "release_lock" in artifacts:
        try:
            ...
            canonical_lock_errors: list[str] = []
            canonical_lock_renderer = getattr(
                release_contract,
                "canonical_release_lock_text",
                None,
            )
            if callable(canonical_lock_renderer):
                try:
                    observed_lock_text = artifacts[
                        "release_lock"
                    ].data.decode("utf-8")
                    expected_lock_text = canonical_lock_renderer(lock)
                except (UnicodeError, TypeError, ValueError) as error:
                    canonical_lock_errors.append(str(error))
                else:
                    if observed_lock_text != expected_lock_text:
                        canonical_lock_errors.append(
                            "release_lock.json is not in canonical "
                            "serialization"
                        )
            _record_check(
                report,
                "release_lock_canonical_bytes",
                not canonical_lock_errors,
                actual=canonical_lock_errors,
                required=(
                    "canonical release-lock serialization when required by "
                    "the selected contract"
                ),
                failure="; ".join(canonical_lock_errors),
            )
            if results_artifact is not None:
                digest_match = (
            ...
            identity_errors = _release_result_source_identity_errors(
                lock,
                macros,
            )
            _record_check(
                report,
                "release_lock_result_source_identity",
                not identity_errors,
                actual=identity_errors,
                required=(
                    "ResultSource exactly identifies the release-lock run "
                    "and cache digest prefix"
                ),
                failure="; ".join(identity_errors),
            )
