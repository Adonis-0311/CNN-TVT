% =====================================================================
% TVT submission freeze, manuscript version V5.5 Final (2026-08-22).
...
% compression to a campaign diagnostic, and sequential attribution controls.
...
\begin{abstract}
Learned wireless receivers often replace information-rich received samples
with compact representations to reduce inference cost, but the preferred
representation can change across interference and mobility regimes. In a source-disjoint
simulation campaign, a sealed whole-configuration contrast improves macro-F1
by \MethodEffectDiff\ \pp, while isolated teacher and route-count effects
remain unresolved. A post-hoc severe-condition analysis reveals a
compact-versus-I/Q ranking reversal inside the campaign, but an independently
generated severe-held evaluation shows that this favorable ordering and a
campaign-derived overlap--SIR diagnostic surface do not transport uniformly.
On the same frozen severe-held benchmark, a lightweight received-I/Q sidecar
produces a consistent \SevereRepairDiff\ \pp\ relative repair at about 3\%
additional MACs. A retrained ablation with the I/Q input
permanently zeroed yields $\RetrainedIqzeroVsAFiveDiff$ \pp\ relative to the
spectral baseline, supporting attribution of the repair to received-I/Q
information rather than to added capacity or fusion structure. All models
remain low in absolute macro-F1 at this extreme operating point, so the result
is interpreted as robustness repair. These
results frame AMC model comparison as a learned-receiver robustness problem:
model ranking is regime-dependent, campaign-derived condition surfaces may
fail to transport, and preserving a small information-rich signal path can
recover part of the exposed degradation.
\end{abstract}
...
The original confirmatory family contains the A5--A0 whole-configuration
contrast and two component contrasts. Rank mapping is exploratory, while the
independent severe-domain transport, repair, and attribution tests were
specified sequentially before their respective evaluations; Section~IV-D
gives the full evidence hierarchy.}}
\end{center}
...
a fixed compute budget, and a marginal score then hides both the rank changes
and the risk that a campaign-internal favorable region will fail under a new
regime. The issue extends beyond AMC: learned wireless receivers often trade
information-rich signal representations for compact front ends under tight
inference budgets, while deployment spans channel, mobility, and interference
regimes not represented by one training campaign. A static leaderboard is
therefore insufficient whenever the preferred representation changes with
operating condition; the more relevant design questions are whether such
ranking relationships transport and whether lost robustness can be recovered
without reverting to a full high-cost receiver.
...
be diagnosed and repaired at the representation level. We answer these
questions using a source-disjoint simulation campaign, an independently
generated severe-domain evaluation, and targeted representation interventions.
The base model, \method, maps a complex STFT to
...
\begin{itemize}
  ...
  teacher-form and route-count effects remain unresolved;
  ...
  \item a \mbox{source-disjoint} independently generated severe-domain evaluation
  ...
  \item on the same frozen severe-held benchmark, a lightweight received-I/Q
  sidecar provides a consistent \SevereRepairDiff\ \pp\ relative repair at
  about 3\% additional MACs. Tested spectral width scaling does not reproduce
  the repair, and a retrained zeroed-I/Q ablation
  supports attribution to received-I/Q information rather than to added
  capacity or fusion structure.
\end{itemize}

Across the frozen campaign, these representations exhibit condition-dependent
rank instability, the apparent severe boundary does not transport uniformly,
and a lightweight received-I/Q path repairs part of the degradation on the
same severe-held benchmark; the retrained zeroed-I/Q ablation identifies
informative received-I/Q input as the enabling factor within the tested
architecture.
...
\cite{taori2020naturalshift,miller2021accuracyline,koh2021wilds}. This work focuses on a
controlled wireless-receiver evidence chain: campaign-level rank instability,
an independently generated severe-domain transport test, and a low-cost
received-I/Q intervention evaluated on the same held domain. The
low-dimensional envelope serves as a diagnostic hypothesis whose transfer is
explicitly tested rather than assumed.
...
Fig.~\ref{fig:teacher} illustrates the method.
...
\subsection{Evidence Hierarchy and Sequential Validation}
The original confirmatory family was fixed before the campaign analysis and
contains the three contrasts of Section~V-A. SNR--SIR mapping,
overlap-envelope analysis, fusion, and capacity studies are treated as
exploratory campaign analyses; the severe SIR stratification is post hoc.

Subsequent severe-domain tests were specified sequentially. The transfer test
was fixed before any inference on the independent severe-held set; the
sidecar-repair analysis was specified after observing the transfer result and
before sidecar evaluation; attribution controls and the retrained zeroed-I/Q
ablation were likewise specified before their respective executions. These
analyses provide prospective post-campaign evidence while remaining distinct
from the original confirmatory family. Detailed plan hashes and execution
provenance are provided in the Supplement.
...
\section{Results}
\subsection{Sealed Whole-Configuration Contrast and Component Tests}
...
interpretation remains campaign-specific. Section~V-F evaluates this
...
Hann-windowed 128-point STFT with hop 32, independent of the
...
A cell is one (split, SIR stratum,
...
floor only at $0$ dB. The primary held set is the
...
outside this primary envelope because a jammer-free window has no
physical finite SIR.
...
correlation bounds collinearity while leaving confounding open.
...
paired intervals including zero). Transport tests then examine the surface's
reach: leaving out the $-15$-dB SIR stratum during fitting does not confirm an
...
constant predictors. Together, these results show that the overlap--SIR
surface summarizes campaign-internal rank variation but has heterogeneous
held-regime skill: for IQFormer-inspired the held-regime
$R^2_{\mathrm{skill}}$ is \RegimeSkillIqSpeed\ (unseen speed),
\RegimeSkillIqChannel\ (held-out channel), \RegimeSkillIqJammer\ (unseen
jammer), and \RegimeSkillIqOod\ (combined OOD), with the corresponding MCLDNN
values \RegimeSkillMcSpeed, \RegimeSkillMcChannel, \RegimeSkillMcJammer, and
\RegimeSkillMcOod\ (Supplement Table~II). The incremental contribution of
overlap beyond SIR is unresolved, and its severe-domain transfer is poor. We
therefore use the surface as a diagnostic summary rather than a
model-selection rule. Figure~\ref{fig:envelope} displays the fit/held cell
structure and the held-cell prediction.
...
campaign-internal rank variation with regime-heterogeneous skill and does not
transport to the independent severe-held regimes (Section~V-F).}
\label{fig:envelope}
\end{figure*}
...
The predefined mechanism expectation was non-increasing gain with overlap; the directional test
...
by \FusionIqformerControl\ and \FusionMcldnnControl\ \pp, leaving excess
cross-model fusion gains of \FusionIqformerExcess\ and \FusionMcldnnExcess\ \pp.
...
to interpretation. The earlier clean-boundary diagnosis
...
An independently generated severe-held set, frozen before inference, then
tests broad transport. SIR is fixed at $-15$ dB, the same level as the
campaign corner cells: R1 combines held jammer families with 180-km/h
mobility over training channels, while R2 combines held channels with the
same jammers at 250 km/h. Relative to the campaign corner, the held set
therefore changes jammer family and speed in R1, and jammer family, speed,
and channel in R2. Each regime spans eight SNRs and
\SevereHeldSourcesPerRegime\ \mbox{source-disjoint} examples
(\SevereHeldSourcesTotal\ total); every checkpoint remains frozen. The
campaign-internal corner does not transfer under either aggregation
convention: IQFormer-inspired and MCLDNN lead A5 by
\SevereReferenceLeadIqformer\ and \SevereReferenceLeadMcldnn\ percentage points
under the seed-wise convention; corresponding window-pooled leads are
\SevereTransferLeadIqformer\ and \SevereTransferLeadMcldnn\ percentage points.
...
A per-family split of the held predictions localizes the degradation: under
OFDM-like interference every evaluated model sits near the floor
($\SevereHeldFamilyOfdmAFive$\% for A5), while the pulse family remains
learnable ($\SevereHeldFamilyPulseAFive$\% for A5,
$\SevereHeldFamilyPulseMcldnn$\% for MCLDNN). The pulse level also explains
why MCLDNN's held level exceeds its campaign $-15$-dB level: the held pulse
family yields substantially higher I/Q-domain performance under this
evaluation, so out-of-domainness and jammer-family difficulty are confounded
in this comparison. Per-family and
per-regime levels are tabulated in the Supplement.
...
On the same frozen severe-held benchmark that exposed the transport failure,
...
incremental compute. The remaining controls are
...
\subsection{Transport Limits of the Diagnostic Envelope}
...
\section{Conclusion}
Compact spectral and high-capacity I/Q AMC representations exhibit
condition-dependent ranking under structured vehicular interference. The
sealed campaign resolves a positive A5--A0 whole-configuration contrast, while
campaign-level analysis reveals a severe-condition rank reversal. An
independently generated severe-domain evaluation shows that this favorable
ordering and the overlap--SIR diagnostic do not transfer uniformly across
unseen regime combinations. On the same benchmark, a lightweight received-I/Q
sidecar recovers \SevereRepairDiff\ percentage points at about 3\% additional
MACs. Wider spectral models do not reproduce the gain, and a retrained
zeroed-I/Q control returns to the spectral baseline, identifying informative
received-I/Q input as the enabling factor within the tested architecture.
Absolute performance remains low in the extreme severe-held regime, so the
reported gain is interpreted as robustness recovery. These results motivate
ranking-stability evaluation and low-cost complementary signal paths for
learned receivers operating under regime shift.
