...
> **Historical v1 admission audit.** Its source/license findings remain
> provenance, but its v1 freeze path and five-seed registration are superseded.
> Current execution and admissibility are controlled by
> `tvt_submission/configs/formal_tvt_freeze_v2.json` and
> `docs/TVT_REVIEW_RESPONSE_MATRIX.md`.

> **2026-08-17 update:** current 2024--2026 metadata, the newly located
> first-author DenoMAE2.0 repository, and the manuscript positioning boundary
> are recorded in `docs/RECENT_LITERATURE_POSITIONING_2026.md`. That update
> supersedes the statement below that the DenoMAE2.0 code destination could not
> be resolved; the repository is public but still lacks the audited license and
> release boundary required for executable admission.
