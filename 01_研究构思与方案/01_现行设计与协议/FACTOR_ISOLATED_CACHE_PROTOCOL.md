...
For the formal v2 queue, the required prospective design decision is now
closed by `TVT_V2_PROSPECTIVE_STATISTICAL_JUSTIFICATION.md` and the
hash-bound `prospective_statistical_design` object in
`tvt_submission/configs/formal_tvt_freeze_v2.json`. That contract deliberately
does not claim a model-based power percentage: no eligible v2 pilot estimate
of algorithm-seed variance or paired source dependence existed before
execution. It instead freezes the SESOI/claim rule, exact seed and
source-cluster counts, three-contrast multiplicity family, interval method,
precision reporting, and stopping/amendment rules.
...
claim. The generic warning remains applicable to new protocols. Formal v2
satisfies the statistical portion through the byte-verified prospective
justification above; its adequacy remains conditional on honestly reporting
the achieved simultaneous-interval precision rather than adding sources or
