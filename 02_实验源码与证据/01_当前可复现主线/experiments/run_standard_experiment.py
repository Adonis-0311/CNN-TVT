...
def available_model_factories() -> dict[str, ModelFactory]:
    ...
    def proportional_teacher_ablation(
        classes: int,
        jammers: int,
        config: ModelConfig,
    ) -> BuiltStandardModel:
        return BuiltStandardModel(
            model=VIMDNet(classes, jammers, config),
            teacher=ProportionalTriMaskTeacher(config),
            objective=TrainingObjective(
                name="tri_mask_proportional_power_teacher",
                loss_family="vimd",
                use_mask_supervision=True,
            ),
    ...
    def local_whitened_teacher_diagnostic(
        classes: int,
        jammers: int,
        config: ModelConfig,
    ) -> BuiltStandardModel:
        return BuiltStandardModel(
            model=VIMDNet(classes, jammers, config),
            teacher=LocalWhitenedTriMaskTeacher(
                config,
                frequency_bins=9,
                time_frames=5,
                whitening_exponent=1.0,
            ),
            objective=TrainingObjective(
                name="diagnostic_tri_mask_local_whitened_margin_teacher",
                loss_family="vimd",
                use_mask_supervision=True,
            ),
            loss_weights=no_auxiliary,
    ...
    factories: dict[str, ModelFactory] = {
        ...
        "a3p_tri_proportional_teacher": proportional_teacher_ablation,
        "a3_ratio_teacher": proportional_teacher_ablation,
        ...
        "diagnostic_a3_local_whitened_teacher": (
            local_whitened_teacher_diagnostic
        ),
    }
    optional = (
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        choices=(BASE_SOURCE_PROFILE, TVT_V2_SOURCE_PROFILE),
        default=BASE_SOURCE_PROFILE,
        help=(
            "Repository-owned source set fingerprinted before and after "
            "execution. Formal TVT-v2 wrappers require tvt_v2_execution."
        ),
    )
...
def source_tree_execution_audit(
    ...
    if not isinstance(profile, str):
        raise ValueError("source-tree start record lacks a profile")
    end = source_tree_record(ROOT, Path(__file__), profile=profile)
    unchanged = (
...
def main() -> None:
    ...
    source_tree_start = source_tree_record(
        ROOT,
        Path(__file__),
        profile=arguments.source_binding_profile,
    )
    ...
    run_record: dict[str, Any] = {
        ...
        "teacher_form_ablation_protocols": TEACHER_FORM_ABLATION_PROTOCOLS,
        "local_normalization_teacher_policy": (
            LOCAL_NORMALIZATION_TEACHER_POLICY
        ),
