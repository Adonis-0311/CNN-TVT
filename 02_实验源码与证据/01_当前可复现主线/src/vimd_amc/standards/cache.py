...
channelized in bounded deterministic MATLAB chunks, audibly cropped, and then
passed through :meth:`SignalSynthesizer.finalize_received_components` for the
shared receiver/noise/SNR/SIR bookkeeping used by the proxy experiments.
"""
...
DEFAULT_MATLAB_BATCH_SIZE = 8192
MAT_V7_MAX_VARIABLE_BYTES = 2**31 - 1
...
@dataclass(frozen=True)
class ReceiverStressSplitPolicy(FactorSplitPolicy):
    """V2-only extension; v1 policy serialization remains byte-compatible."""

    adc_bits: int | None = None
    adc_full_scale: float = 3.0
    per_emitter_cfo_norm_max: float = 0.0
    per_emitter_timing_offset_max_samples: int = 0
...
def factor_isolated_split_policies_v2(
    split_sizes: Mapping[str, int] | None = None,
) -> tuple[FactorSplitPolicy, ...]:
    """Return the v1 factors plus three receiver-robustness test splits.

    The stress splits reuse the hard-interference factor support.  The only
    isolated changes are post-AGC ADC precision or independent pre-mixing
    emitter synchronization errors.  They are source-disjoint from every
    primary split and from one another.
    """

    sizes = {split: 10 for split in TVT_V2_FACTOR_SPLITS}
    if split_sizes is not None:
        unknown = set(split_sizes).difference(TVT_V2_FACTOR_SPLITS)
        if unknown:
            raise ValueError(
                f"unknown v2 split sizes: {sorted(unknown)}"
            )
        sizes.update(
    ...
    primary = list(
        factor_isolated_split_policies(
            {split: sizes[split] for split in FACTOR_ISOLATED_SPLITS}
        )
    )
    ...
    def stress_policy(
        ...
        *,
        ...
        return ReceiverStressSplitPolicy(
            split=split,
            role=role,
            size=sizes[split],
            source_key=f"factor_isolated_v2::{split}",
            profiles=_SEEN_PROFILES,
            jammer_choices=_SEEN_JAMMERS,
            speeds_kmh=_SEEN_SPEEDS_KMH,
            snr_db_values=_STANDARD_SNR_DB_VALUES,
            sir_db_values=_HARD_SIR_DB_VALUES,
            clean_fraction=0.0,
            held_factors=(),
            isolation_factors=(
                ("adc_precision",)
                if adc_bits is not None
                else ("emitter_synchronization",)
            ),
            adc_bits=adc_bits,
            adc_full_scale=3.0,
            per_emitter_cfo_norm_max=per_emitter_cfo_norm_max,
    ...
    return (
        *primary,
        stress_policy(
            "adc_10bit_agc",
            "hard_interference_receiver_robustness_test",
            adc_bits=10,
        ),
        stress_policy(
            "adc_12bit_agc",
        ...
        stress_policy(
            "per_emitter_sync",
            "hard_interference_per_emitter_sync_robustness_test",
...
class TDLCacheBuildConfig:
    ...
    def _validate_split_policies(self, split_sizes: Mapping[str, int]) -> None:
        ...
        for split, policy in policies.items():
            ...
            adc_bits = getattr(policy, "adc_bits", None)
            adc_full_scale = float(
                getattr(policy, "adc_full_scale", 3.0)
            )
            per_emitter_cfo_norm_max = float(
                getattr(policy, "per_emitter_cfo_norm_max", 0.0)
            )
            per_emitter_timing_offset_max_samples = int(
                getattr(
                    policy,
                    "per_emitter_timing_offset_max_samples",
                    0,
            ...
            if adc_bits is not None and (
                isinstance(adc_bits, bool)
                or not isinstance(adc_bits, int)
                or adc_bits < 2
                or adc_bits > 24
            ):
                raise ValueError(f"{split} adc_bits must be in [2, 24].")
            if (
                not np.isfinite(adc_full_scale)
                or adc_full_scale <= 0.0
            ):
                raise ValueError(
                    f"{split} adc_full_scale must be positive and finite."
                )
            if (
                not np.isfinite(per_emitter_cfo_norm_max)
                or per_emitter_cfo_norm_max < 0.0
                or per_emitter_cfo_norm_max >= 0.5
            ):
                raise ValueError(
                    f"{split} per-emitter CFO range must be in [0, 0.5)."
                )
            if (
                isinstance(
                    getattr(
                ...
                or per_emitter_timing_offset_max_samples < 0
                or per_emitter_timing_offset_max_samples
                >= self.sample_length
            ):
                raise ValueError(
                    f"{split} per-emitter timing range is invalid."
                )
...
def cache_build_execution_preflight(
    config: TDLCacheBuildConfig,
    *,
    matlab_batch_size: int = DEFAULT_MATLAB_BATCH_SIZE,
) -> dict[str, int | bool | str]:
    """Validate and summarize the bounded MATLAB transfer plan.

    ``matlab_batch_size`` is an execution-only control.  It is intentionally
    excluded from :class:`TDLCacheBuildConfig` and therefore cannot alter the
    frozen simulation configuration, sample order, named seeds, or manifest
    digest.  The bound below applies to each complex-double waveform matrix
    transferred through the MATLAB v7 bridge.
    """

    config.validate()
    if (
        isinstance(matlab_batch_size, bool)
        or not isinstance(matlab_batch_size, int)
        or matlab_batch_size <= 0
    ):
        raise ValueError("matlab_batch_size must be a positive integer")
    total_source_count = sum(size for _, size in config.split_sizes)
    total_view_count = 2 * total_source_count
    extended_sample_length = (
        config.sample_length + 2 * config.guard_samples
    )
    bytes_per_waveform = (
        extended_sample_length * np.dtype(np.complex128).itemsize
    )
    maximum_chunk_view_count = min(matlab_batch_size, total_view_count)
    # One nonzero sentinel may be appended when a contiguous chunk contains
    # only clean jammer waveforms.  It is discarded before cache materialization.
    maximum_transfer_view_count = maximum_chunk_view_count + 1
    maximum_transfer_bytes = (
        maximum_transfer_view_count * bytes_per_waveform
    )
    if maximum_transfer_bytes > MAT_V7_MAX_VARIABLE_BYTES:
        maximum_safe_batch_size = (
            MAT_V7_MAX_VARIABLE_BYTES // bytes_per_waveform - 1
        )
        raise ValueError(
            "matlab_batch_size would exceed the MATLAB v7 single-variable "
            "limit: "
            f"batch_size={matlab_batch_size}, "
            f"maximum_safe_batch_size={maximum_safe_batch_size}, "
            f"bytes_per_waveform={bytes_per_waveform}"
        )
    chunk_count = (
        total_view_count + matlab_batch_size - 1
    ) // matlab_batch_size
    unbatched_transfer_bytes = total_view_count * bytes_per_waveform
    return {
        "execution_mode": "deterministic_streaming_matlab_chunks",
        "total_source_count": total_source_count,
        "total_view_count": total_view_count,
        "extended_sample_length": extended_sample_length,
        "bytes_per_complex128_waveform": bytes_per_waveform,
        "matlab_batch_size": matlab_batch_size,
        "matlab_chunk_count": chunk_count,
        "matlab_process_invocation_count": 2 * chunk_count,
        "maximum_chunk_view_count": maximum_chunk_view_count,
        "maximum_matlab_transfer_view_count": (
            maximum_transfer_view_count
        ),
        "maximum_single_transfer_bytes": maximum_transfer_bytes,
        "unbatched_single_transfer_bytes": unbatched_transfer_bytes,
        "mat_v7_max_variable_bytes": MAT_V7_MAX_VARIABLE_BYTES,
        "unbatched_transfer_exceeds_mat_v7_limit": (
            unbatched_transfer_bytes > MAT_V7_MAX_VARIABLE_BYTES
        ),
        "chunk_transfer_within_mat_v7_limit": True,
        "batch_size_affects_manifest_digest": False,
    }
...
def _build_source_groups(
    ...
    """Build the complete lightweight source-ID plan without waveforms."""
...
def _iter_pending_views(
    synthesizer: SignalSynthesizer,
    config: TDLCacheBuildConfig,
    source_groups: Mapping[str, Sequence[int]],
) -> Iterable[_PendingView]:
    """Yield views in the frozen split/index/view order."""

    extended_length = config.sample_length + 2 * config.guard_samples
    for split, size in config.split_sizes:
        policy = config.policy_for_split(split)
        source_ids = source_groups[split]
        if len(source_ids) != size:
            raise ValueError(
                f"source-ID plan for {split} has {len(source_ids)} rows; "
                f"expected {size}"
            )
...
def _build_pending_views(
    ...
    """Materialize the legacy helper for tests and small diagnostic callers."""

    source_groups = _build_source_groups(config)
    pending = list(
        _iter_pending_views(synthesizer, config, source_groups)
    )
    return pending, source_groups


def _pending_view_chunks(
    pending: Iterable[_PendingView],
    batch_size: int,
) -> Iterable[list[_PendingView]]:
    """Yield bounded, contiguous chunks without changing item order."""

    chunk: list[_PendingView] = []
    for item in pending:
        chunk.append(item)
...
def _channelized_pending_views(
    pending: Iterable[_PendingView],
    *,
    batch_size: int,
    sample_rate_hz: float,
    matlab_executable: str | Path | None,
    ...
    """Channelize bounded contiguous chunks while preserving global order."""

    for chunk in _pending_view_chunks(pending, batch_size):
        sentinel_waveform = chunk[0].source_with_guard
        target_batch = np.stack(
            [item.source_with_guard for item in chunk],
            axis=0,
        )
        target_result = apply_nrtdl_batch(
            target_batch,
            [item.target_configuration for item in chunk],
            sample_rate_hz=sample_rate_hz,
            matlab_executable=matlab_executable,
            timeout_s=matlab_timeout_s,
        )
        del target_batch
        if len(target_result.metadata) != len(chunk):
            raise RuntimeError(
                "MATLAB target chunk returned an unexpected metadata count"
            )
        jammer_batch = np.stack(
            [item.jammer_with_guard for item in chunk],
            axis=0,
        )
        jammer_configurations = [
            item.jammer_configuration for item in chunk
        ]
        if not jammer_batch.any():
            # The bridge deliberately rejects a completely zero input.  A
            # contiguous clean-retention chunk is nevertheless legitimate.
            # Append one deterministic nonzero sentinel, then discard its
            # output and metadata.  Every real zero waveform retains its own
            # independently seeded channel configuration and exact zero
            # output; the sentinel is not written or hashed.
            jammer_batch = np.concatenate(
                (jammer_batch, sentinel_waveform[None, :]),
                axis=0,
            )
            jammer_configurations.append(chunk[0].target_configuration)
        ...
        del jammer_batch
        if len(jammer_result.metadata) != len(jammer_configurations):
            raise RuntimeError(
                "MATLAB jammer chunk returned an unexpected metadata count"
            )
        for local_index, item in enumerate(chunk):
            yield (
                item,
                target_result.waveforms[local_index],
                jammer_result.waveforms[local_index],
                target_result.metadata[local_index],
                jammer_result.metadata[local_index],
            )
...
def build_tdl_paired_cache(
    ...
    *,
    ...
    source_groups = _build_source_groups(build_config)
    split_arrays = {
    ...
    quality_doppler_hz = {
        split: np.empty((size, 2), dtype=np.float64)
        for split, size in build_config.split_sizes
    }
    ...
    pending = _iter_pending_views(
        synthesizer,
        build_config,
        source_groups,
    )
    channelized = _channelized_pending_views(
        pending,
        batch_size=matlab_batch_size,
        sample_rate_hz=build_config.sample_rate_hz,
        matlab_executable=matlab_executable,
    ...
    for (
        ...
        sample = synthesizer.finalize_received_components(
            clean=target_waveform[crop_start:crop_stop],
            raw_jammer=jammer_waveform[crop_start:crop_stop],
            ...
            receiver_stress=(
                ...
                if (
                    (policy := build_config.policy_for_split(item.split))
                    is not None
                    and (
                        getattr(policy, "adc_bits", None) is not None
                        or getattr(
                            policy,
                            "per_emitter_cfo_norm_max",
                            0.0,
        ...
        quality_doppler_hz[item.split][row, view_index] = (
            item.target_configuration.maximum_doppler_hz
        )
    ...
    if processed_view_count != execution_preflight["total_view_count"]:
        raise RuntimeError(
            "streamed MATLAB execution produced an incomplete view grid: "
            f"observed={processed_view_count}, "
            f"expected={execution_preflight['total_view_count']}"
        )

    max_doppler_hz = max(
        float(np.max(values))
        for values in quality_doppler_hz.values()
    )
    for split, values in quality_doppler_hz.items():
        split_arrays[split]["quality"][:, :, 2] = np.clip(
            values / max(max_doppler_hz, 1e-12),
            0.0,
            _QUALITY_CLIP[1],
        )
