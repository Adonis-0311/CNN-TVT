...
class SignalSynthesizer:
    ...
    def finalize_received_components(
        ...
        *,
        ...
        receiver_stress: dict[str, Any] | None = None,
    ) -> SampleComponents:
        """Apply emitter/receiver effects, exact SNR/SIR scaling, and AGC.
        ...
        ``receiver_stress`` is an explicitly manifested robustness-only
        protocol.  Independent CFO and integer timing offsets are applied to
        the target and jammer before mixing.  ADC quantization is applied
        after window-RMS AGC, and its exact residual is retained in
        ``receiver_artifact`` so the component identity remains auditable.
        """
        ...
        stress_rng = _named_rng(condition_seed, "receiver_stress")
        stress = dict(receiver_stress or {})
        allowed_stress_keys = {
            "adc_bits",
            "adc_full_scale",
        ...
        unknown_stress_keys = set(stress).difference(allowed_stress_keys)
        if unknown_stress_keys:
            raise ValueError(
                "receiver_stress contains unsupported keys: "
                f"{sorted(unknown_stress_keys)}"
            )
        adc_bits = stress.get("adc_bits")
        if adc_bits is not None and (
            isinstance(adc_bits, bool)
            or not isinstance(adc_bits, (int, np.integer))
            or int(adc_bits) < 2
            or int(adc_bits) > 24
        ):
            raise ValueError("receiver_stress.adc_bits must be in [2, 24]")
        adc_bits = int(adc_bits) if adc_bits is not None else None
        adc_full_scale = float(stress.get("adc_full_scale", 3.0))
        if not np.isfinite(adc_full_scale) or adc_full_scale <= 0.0:
            raise ValueError(
                "receiver_stress.adc_full_scale must be positive and finite"
            )
        per_emitter_cfo_max = float(
            stress.get("per_emitter_cfo_norm_max", 0.0)
        )
        if (
            not np.isfinite(per_emitter_cfo_max)
            or per_emitter_cfo_max < 0.0
            or per_emitter_cfo_max >= 0.5
        ):
            raise ValueError(
                "receiver_stress.per_emitter_cfo_norm_max must be in [0, 0.5)"
            )
        timing_max = stress.get(
            "per_emitter_timing_offset_max_samples",
            0,
        )
        if (
            isinstance(timing_max, bool)
            or not isinstance(timing_max, (int, np.integer))
            or int(timing_max) < 0
            or int(timing_max) >= len(clean)
        ):
            raise ValueError(
                "receiver_stress.per_emitter_timing_offset_max_samples must "
                "be a nonnegative integer smaller than the sample length"
            )
        timing_max = int(timing_max)
        ...
        target_timing_offset = (
            int(stress_rng.integers(-timing_max, timing_max + 1))
            if timing_max
            else 0
        )
        jammer_timing_offset = (
            int(stress_rng.integers(-timing_max, timing_max + 1))
            if timing_max
            else 0
        )
        sample_index = np.arange(len(clean), dtype=np.float64)
        clean = shift_without_wrap(clean, target_timing_offset) * np.exp(
            1j * 2.0 * np.pi * target_emitter_cfo * sample_index
        )
        raw_jammer = shift_without_wrap(
            raw_jammer,
            jammer_timing_offset,
        ) * np.exp(
            1j * 2.0 * np.pi * jammer_emitter_cfo * sample_index
        )
        if signal_power(clean) <= EPS:
            raise ValueError(
                "per-emitter timing stress removed all target energy"
            )
        ...
        quantization_residual = np.zeros_like(mixture)
        clipping_fraction = 0.0
        quantization_step = 0.0
        if adc_bits is not None:
            quantization_levels = 2**adc_bits
            quantization_step = (
                2.0 * adc_full_scale / (quantization_levels - 1)
            )
            stacked = np.stack((mixture.real, mixture.imag), axis=0)
            clipped = np.clip(
                stacked,
                -adc_full_scale,
                adc_full_scale,
            ...
            clipping_fraction = float(
                np.mean(np.abs(stacked) > adc_full_scale)
            )
            quantized = (
                np.round(
                    (clipped + adc_full_scale) / quantization_step
                )
                * quantization_step
                - adc_full_scale
            )
            quantized_complex = quantized[0] + 1j * quantized[1]
            quantization_residual = quantized_complex - mixture
            mixture = quantized_complex
            receiver_artifact = receiver_artifact + quantization_residual
        ...
        metadata: dict[str, Any] = {
            ...
            "cfo_norm": cfo_norm,
            "shared_receiver_cfo_norm": cfo_norm,
            "target_emitter_cfo_norm": target_emitter_cfo,
            "jammer_emitter_cfo_norm": jammer_emitter_cfo,
            "target_emitter_timing_offset_samples": target_timing_offset,
            "jammer_emitter_timing_offset_samples": jammer_timing_offset,
            "iq_gain_db": iq_gain_db,
            "iq_phase_deg": iq_phase_deg,
            "window_rms_normalization_gain": float(1.0 / agc),
            "agc_mode": "window_rms_before_adc",
            "adc_bits": adc_bits,
            "adc_full_scale": (
                adc_full_scale if adc_bits is not None else None
            ),
            "adc_quantization_step": (
                quantization_step if adc_bits is not None else None
            ),
            "adc_clipping_fraction_iq_entries": clipping_fraction,
            "adc_quantization_residual_power": signal_power(
                quantization_residual
            ),
            "receiver_stress_applied_pre_mixing_per_emitter": bool(
                per_emitter_cfo_max > 0.0 or timing_max > 0
            ),
