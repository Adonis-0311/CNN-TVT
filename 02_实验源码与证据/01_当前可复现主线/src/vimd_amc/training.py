...
def validation_loss(
    ...
    *,
    ...
    for batch in loader:
        ...
        classes = int(output["logits"].shape[1])
        ...
        confusion = (
            ...
            else confusion + batch_confusion
        )
        count += len(labels)
    loss_value = loss_total / max(1, count)
    accuracy = correct / max(1, count)
    if not include_macro_f1:
    ...
    if confusion is None:
        ...
        recall = torch.where(
            support > 0,
            true_positive / support.clamp_min(1.0),
            torch.zeros_like(true_positive),
        )
        precision = torch.where(
            predicted > 0,
            true_positive / predicted.clamp_min(1.0),
            torch.zeros_like(true_positive),
        )
        f1 = torch.where(
            precision + recall > 0,
...
def train_model(
    *,
    ...
    for epoch in range(config.epochs):
        ...
        sample_count = 0
        train_confusion: torch.Tensor | None = None
        mask_factor = float(
            np.clip(
        ...
        for batch in train_loader:
            ...
            predictions = first_output["logits"].detach().argmax(dim=1)
            classes = int(first_output["logits"].shape[1])
            batch_confusion = torch.bincount(
                labels * classes + predictions,
                minlength=classes * classes,
            ).reshape(classes, classes)
            train_confusion = (
                ...
                else train_confusion + batch_confusion
            )
            for name, value in losses.items():
                running[name] = running.get(name, 0.0) + float(value.detach()) * batch_size

        scheduler.step()
        val_loss, val_accuracy, val_macro_f1 = validation_loss(
            model,
            validation_loader,
            device,
        ...
        if train_confusion is None:
            ...
            matrix = train_confusion.to(dtype=torch.float64)
            true_positive = torch.diag(matrix)
            support = matrix.sum(dim=1)
            predicted = matrix.sum(dim=0)
            recall = torch.where(
            ...
            train_macro_f1 = float(
                train_f1[represented].mean().item()
                if bool(represented.any())
                else 0.0
            )
        record = {
            ...
            **{name: value / max(1, sample_count) for name, value in running.items()},
            "training_total_loss": (
                running.get("total", 0.0) / max(1, sample_count)
            ),
