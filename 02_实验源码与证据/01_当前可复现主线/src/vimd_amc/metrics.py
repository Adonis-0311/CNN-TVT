...
def ood_axis_calibrated_bootstrap(
    id_baselines_by_seed: Mapping[Any, PredictionBundle],
    ood_baselines_by_seed: Mapping[Any, PredictionBundle],
    ood_candidates_by_seed: Mapping[Any, PredictionBundle],
    *,
    draws: int = 10_000,
    seed: int = 20260803,
    recovery_fraction: float = 0.25,
    noninferiority_floor: float = -0.01,
    opportunity_floor: float = 0.005,
) -> dict[str, Any]:
    """Calibrate one OOD gate to the opportunity exposed by the A0 baseline.

    The ID and OOD source sets are intentionally source-disjoint and are
    resampled independently within modulation class.  Within the OOD split,
    baseline and candidate predictions remain strictly paired.  Algorithm
    seeds are resampled jointly across all three quantities.

    If the baseline's ID-to-OOD degradation has a lower 95% bound above
    ``opportunity_floor``, the axis is consequential and the candidate must
    recover ``recovery_fraction`` of the degradation with a positive lower
    confidence bound.  Otherwise no improvement claim is licensed; the
    candidate must instead satisfy the predeclared noninferiority floor.
    """
    ...
    if noninferiority_floor > 0.0:
        raise ValueError("noninferiority_floor must be nonpositive")
    if opportunity_floor < 0.0:
        raise ValueError("opportunity_floor must be nonnegative")
    key_sets = (
        set(id_baselines_by_seed),
        set(ood_baselines_by_seed),
        set(ood_candidates_by_seed),
    )
    if not key_sets[0] or key_sets[0] != key_sets[1] or key_sets[0] != key_sets[2]:
        raise ValueError(
            "ID baseline, OOD baseline, and OOD candidate mappings must "
            "have the same nonempty algorithm-seed keys"
        )
    seed_keys = sorted(key_sets[0], key=lambda value: str(value))
    first_id = id_baselines_by_seed[seed_keys[0]]
    first_ood_baseline = ood_baselines_by_seed[seed_keys[0]]
    first_ood_candidate = ood_candidates_by_seed[seed_keys[0]]
    first_id.validate()
    classes = _validate_paired_bundles(
        first_ood_baseline,
        first_ood_candidate,
    )
    if int(first_id.probabilities.shape[1]) != classes:
        raise ValueError("ID and OOD class taxonomies differ")
    id_labels = np.asarray(first_id.labels, dtype=np.int64)
    id_sources = np.asarray(first_id.source_ids)
    ood_labels = np.asarray(first_ood_baseline.labels, dtype=np.int64)
    ood_sources = np.asarray(first_ood_baseline.source_ids)
    id_predictions: list[np.ndarray] = []
    ood_baseline_predictions: list[np.ndarray] = []
    ood_candidate_predictions: list[np.ndarray] = []
    for algorithm_seed in seed_keys:
        id_bundle = id_baselines_by_seed[algorithm_seed]
        ood_baseline = ood_baselines_by_seed[algorithm_seed]
        ood_candidate = ood_candidates_by_seed[algorithm_seed]
        id_bundle.validate()
        if int(id_bundle.probabilities.shape[1]) != classes:
            raise ValueError("ID class taxonomy differs across seeds")
        if not np.array_equal(id_bundle.labels, id_labels) or not np.array_equal(
            id_bundle.source_ids,
            id_sources,
        ):
            raise ValueError("ID source IDs, order, or labels differ across seeds")
        if _validate_paired_bundles(ood_baseline, ood_candidate) != classes:
            raise ValueError("OOD class taxonomy differs across seeds")
        if not np.array_equal(
            ood_baseline.labels,
            ood_labels,
        ) or not np.array_equal(ood_baseline.source_ids, ood_sources):
            raise ValueError(
                "OOD source IDs, order, or labels differ across seeds"
            )
        id_predictions.append(id_bundle.predictions)
        ood_baseline_predictions.append(ood_baseline.predictions)
        ood_candidate_predictions.append(ood_candidate.predictions)

    id_strata, id_cluster_count = _cluster_strata(
        id_labels,
        id_sources,
        stratify_by_class=True,
    )
    ood_strata, ood_cluster_count = _cluster_strata(
        ood_labels,
        ood_sources,
        stratify_by_class=True,
    )
    seed_count = len(seed_keys)
    all_id_indices = np.arange(len(id_labels))
    all_ood_indices = np.arange(len(ood_labels))

    def statistics(
        selected_seeds: np.ndarray,
        selected_id_sources: np.ndarray,
        selected_ood_sources: np.ndarray,
    ) -> tuple[float, float, float]:
        id_scores: list[float] = []
        baseline_ood_scores: list[float] = []
        candidate_ood_scores: list[float] = []
        for seed_index in selected_seeds:
            index = int(seed_index)
            id_scores.append(
                _macro_f1(
                    id_labels[selected_id_sources],
                    id_predictions[index][selected_id_sources],
                    classes,
                )
            )
            baseline_ood_scores.append(
                _macro_f1(
                    ood_labels[selected_ood_sources],
                    ood_baseline_predictions[index][selected_ood_sources],
                    classes,
                )
            )
            candidate_ood_scores.append(
                _macro_f1(
                    ood_labels[selected_ood_sources],
                    ood_candidate_predictions[index][selected_ood_sources],
                    classes,
                )
            )
        degradation = float(
            np.mean(id_scores) - np.mean(baseline_ood_scores)
        )
        gain = float(
            np.mean(candidate_ood_scores) - np.mean(baseline_ood_scores)
        )
        calibrated_margin = gain - recovery_fraction * max(
            degradation,
            0.0,
        )
        return degradation, gain, calibrated_margin
    ...
    for draw in range(draws):
        ...
        selected_id = _sample_cluster_indices(rng, id_strata)
        selected_ood = _sample_cluster_indices(rng, ood_strata)
        samples[draw] = statistics(
            selected_seeds,
            selected_id,
            selected_ood,
        )
    intervals = [
        _confidence_interval(samples[:, index]) for index in range(3)
    ]
    degradation_low, degradation_high = intervals[0]
    gain_low, gain_high = intervals[1]
    margin_low, margin_high = intervals[2]
    consequential = degradation_low > opportunity_floor
    axis_gate_passed = (
        margin_low > 0.0
        if consequential
        else gain_low >= noninferiority_floor
    )
    return {
        "a0_id_to_ood_macro_f1_degradation": observed[0],
        "a0_id_to_ood_macro_f1_degradation_ci95_low": degradation_low,
        "a0_id_to_ood_macro_f1_degradation_ci95_high": degradation_high,
        "candidate_minus_a0_ood_macro_f1_gain": observed[1],
        "candidate_minus_a0_ood_macro_f1_gain_ci95_low": gain_low,
        "candidate_minus_a0_ood_macro_f1_gain_ci95_high": gain_high,
        "calibrated_recovery_margin": observed[2],
        "calibrated_recovery_margin_ci95_low": margin_low,
        "calibrated_recovery_margin_ci95_high": margin_high,
        "opportunity_floor": float(opportunity_floor),
        "recovery_fraction": float(recovery_fraction),
        "noninferiority_floor": float(noninferiority_floor),
        "axis_classification": (
            "consequential_baseline_degradation"
            if consequential
            else "no_confirmed_consequential_baseline_degradation"
        ),
        "axis_gate_rule": (
            "calibrated_recovery_margin_ci95_low>0"
            if consequential
            else "candidate_minus_a0_gain_ci95_low>=noninferiority_floor"
        ),
        "axis_gate_passed": bool(axis_gate_passed),
        "bootstrap_draws": int(draws),
        "bootstrap_seed": int(seed),
        "algorithm_seed_count": int(seed_count),
        "algorithm_seed_ids": [str(value) for value in seed_keys],
        "id_test_source_cluster_count": int(id_cluster_count),
        "ood_test_source_cluster_count": int(ood_cluster_count),
        "bootstrap_hierarchy": (
            "joint_algorithm_seed_with_independent_class_stratified_"
            "id_and_ood_source_clusters"
        ),
        "source_alignment": (
            "ID/OOD source-disjoint; OOD candidate/A0 strictly paired"
        ),
    }
