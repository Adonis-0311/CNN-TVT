...
class PaperBuildGateTest(unittest.TestCase):
        ...
        selected_inputs = (
            [
                ...
                *(
                    ...
                    if (paper_root / "authors_verified.tex").is_file()
                    else []
                ),
            ]
            if inputs is None
    ...
    def _write_gate_main(
        ...
        *,
        ...
        authors_path = paper_root / "authors_verified.tex"
        if not internal_review and not authors_path.exists():
    ...
    def _fixture(
        ...
        *,
        ...
        if not internal_review:
            (paper_root / "authors_verified.tex").write_text(
                "\\author{Fixture Author}\n",
                encoding="utf-8",
            )
    ...
    def test_decoy_main_and_release_lock_overrides_are_rejected(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory(
            prefix="vimd_paper_decoy_canonical_paths_"
        ) as temporary:
            paper_root = self._fixture(
                Path(temporary),
                internal_review=False,
            ...
            decoy_main = paper_root / "decoy_main.tex"
            decoy_main.write_bytes(main.read_bytes())
            main.write_text(
                "\\documentclass{article}\n"
                "\\begin{document}unaudited\\end{document}\n",
                encoding="utf-8",
            )
            lock = paper_root / "release_lock.json"
            decoy_lock = paper_root / "decoy_lock.json"
            decoy_lock.write_bytes(lock.read_bytes())
            lock.write_text("{}\n", encoding="utf-8")
            self._write_fls(
                paper_root,
                inputs=[
            ...
            report = gate.audit_paper_build(
                paper_root=paper_root,
                mode="release",
                tex_path=Path(decoy_main.name),
                release_lock_path=Path(decoy_lock.name),
            )
            checks = self._checks(report)
            self.assertFalse(report["ok"])
            self.assertFalse(checks["main_path_binding"]["passed"])
            self.assertFalse(
                checks["release_lock_path_binding"]["passed"]
            )

    def test_release_rejects_placeholder_author_metadata(self) -> None:
        with tempfile.TemporaryDirectory(
            prefix="vimd_paper_placeholder_authors_"
        ) as temporary:
            paper_root = self._fixture(
                Path(temporary),
                internal_review=False,
            ...
            self.assertFalse(report["ok"])
            self.assertFalse(
                checks["release_author_metadata"]["passed"]
            )
