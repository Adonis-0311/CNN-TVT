...
from tvt_submission import validate_paper_build as paper_gate
from tvt_submission import validate_v2_paper_release as v2_release
...
class AblationEvidenceContractTest(unittest.TestCase):
    ...
    def test_checked_in_placeholder_and_table_three_wire_all_new_macros(
        ...
        self.addCleanup(
            paper_gate._configure_release_contract,
            paper_gate.LEGACY_RELEASE_CONTRACT,
        )
        paper_gate._select_release_contract(results_text)
        self.assertIs(paper_gate.release_contract, v2_release)
        self.assertEqual(len(paper_gate.PROVENANCE_MACROS), 121)
        ...
        self.assertEqual(
            ...
            set(paper_gate.PROVENANCE_MACROS),
        )
        for name in (
            *paper_gate.ABLATION_MEAN_NUMERIC_MACROS,
            *paper_gate.ABLATION_CONTRAST_NUMERIC_MACROS,
        ):
