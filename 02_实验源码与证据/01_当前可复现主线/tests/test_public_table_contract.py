...
class PublicTableContractTest(unittest.TestCase):
    def test_checked_in_placeholder_has_exact_interface_and_wiring(self) -> None:
        ...
        self.addCleanup(
            gate._configure_release_contract,
            gate.LEGACY_RELEASE_CONTRACT,
        )
        gate._select_release_contract(result_text)
        self.assertIs(gate.release_contract, v2_release)
        self.assertEqual(len(gate.PROVENANCE_MACROS), 121)
        self.assertEqual(len(gate.RESULT_MACROS), 122)
        self.assertEqual(len(gate.release_contract.ALL_MACROS), 123)
        self.assertEqual(
