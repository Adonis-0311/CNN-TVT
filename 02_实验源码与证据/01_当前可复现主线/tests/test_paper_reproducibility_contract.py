...
class PaperReproducibilityContractTest(unittest.TestCase):
    def test_frozen_protocol_table_and_prose_follow_source_contract(self) -> None:
        ...
        self.assertEqual(cache["preset"], "headline_v2")
        self.assertEqual(split_counts, builder_sizes["headline_v2"])
        self.assertEqual(len(models), 12)
        self.assertIn("a3p_tri_proportional_teacher", models)
        ...
        view_slots = _literal_for_iterable(
            CACHE_PATH, "_iter_pending_views", "view"
        )
        view_count = source_count * len(view_slots)
        fit_count = len(models) * len(seeds)
        self.assertEqual(
            (source_count, view_count, fit_count),
            (152_000, 304_000, 120),
        )
        ...
        expected_cells = {
            ...
            "Linear waveforms": (
                r"Fixed 4 samples/symbol (\(R_s=250\)~ksym/s); "
                r"energy-normalized RRC, roll-off 0.35, span 8 symbols; "
                r"integer timing phase \(0,\ldots,3\) randomized per source"
            ),
            "Continuous phase": (
                r"GMSK: \(h=0.5\), Gaussian \(\sigma=0.55T_s\), truncated "
                r"to \(\pm4T_s\) (nominal \(BT\approx0.24\)); CPFSK: "
                r"\(h=0.5\); 4FSK offsets "
                r"\(\{-0.36,-0.12,0.12,0.36\}R_s\). Parameters fixed; "
                "data and initial phase randomized"
            ),
            "Channel/mobility": (
            ...
            "Source budget": (
                ...
                f"{split_counts['validation']:,}; each of ten test splits "
                f"{split_counts['id_test']:,}; {source_count:,} disjoint "
                f"source IDs and {view_count:,} cached views; preregistered "
                "A0/A5 scale check at 10k/30k/100k precedes the "
                "confirmatory run, with 100k fixed regardless of that curve"
            ),
            "Transform/widths": (
                f"Periodic-Hann STFT {model['n_fft']}/"
                f"{model['hop_length']} (FFT/hop), "
                r"\texttt{center=false}, unitary normalization, full "
                "two-sided spectrum, 61 frames; "
            ...
            "Execution matrix": (
                r"A0--A7 plus A3\(^{\prime}\), MCLDNN, IQFormer-inspired, "
                "and CSSL adaptation; "
                r"seeds $\{" + ",".join(str(seed) for seed in seeds)
                + rf"\}}$; {len(models)} models $\times$ {len(seeds)} seeds "
                + rf"= {fit_count} confirmatory fits; {reference_label} "
                "is the fixed primary anchor"
            ),
            "Inference/statistics": (
                f"one view; {statistics['bootstrap_draws']:,}-draw seed/source "
                f"hierarchical paired bootstrap "
                f"(seed {statistics['bootstrap_seed']}, distinct from the "
                "generator seed); class-stratified "
        ...
        reproducibility_index = main.index(
            r"\section{Reproducibility and Scope}"
        )
        ...
        self.assertIn(
            "Paired A5-minus-Reference",
            main,
        )
        self.assertIn(
            "A0 Is the Reference for Hard, OOD, and Receiver",
            main,
        )
        self.assertIn(
            "CSSL Adaptation Is the Reference for Clean Retention",
            main,
        )
        self.assertNotIn("after paired multiplicity", main)
        self.assertIn(
            r"Restricted to A5--A0, A3--A3\(^{\prime}\), and A5--A6",
            main,
        )
        ...
        self.assertIn("over the shared A0 backbone", conclusion)
        self.assertNotIn(r"\PrimaryReference", conclusion)
        self.assertIn(
